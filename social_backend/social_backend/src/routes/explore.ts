...
function getViewer(req: any): Viewer {
  // 开发阶段：用 header 模拟登录用户
  // Flutter 请求时可加：x-user-id / x-user-name
...
function parseCategory(v: any): ExploreCategory {
  const s = (v || "").toString();
  if (s === "galaxy" || s === "station" || s === "planet") return s as ExploreCategory;
  ...
  router.post("/posts", async (req, res) => {
    try {
      ...
      const imageUrls: string[] = Array.isArray(req.body.imageUrls)
        ? req.body.imageUrls.map((x: any) => String(x)).filter((x: string) => x.trim().length > 0)
        : [];

      const title =
        content.trim().length === 0 ? "新动态" : content.trim().split("\n")[0].slice(0, 80);
  ...
  router.post("/posts/:id/like", async (req, res) => {
    ...
    try {
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (existing) {
          await tx.like.delete({ where: { postId_userId: { postId, userId: viewer.id } } });
  ...
  router.get("/posts/:id/comments", async (req, res) => {
    try {
      ...
      const items = slice.map((c) => ({
        ...
        content: c.content,
        createdAtMs: toMs(c.createdAt), // ✅ 对齐 Flutter CommentModel(createdAtMs)
      }));
  ...
  router.post("/posts/:id/comments", async (req, res) => {
    ...
    try {
      ...
      res.json({
        ...
        createdAtMs: toMs(created.createdAt), // ✅ 对齐
      });
    } catch (e: any) {
      res.status(500).json({ error: e?.message || String(e) });
    }
